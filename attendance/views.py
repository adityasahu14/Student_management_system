...
class StudentDashboardView(StudentMixin, View):
    def get(self, request):
        ...
        cumulative = get_cumulative_attendance(student, start_date, end_date)
        show_warning = cumulative < LOW_ATTENDANCE_THRESHOLD and cumulative > 0
        
        subjects = Subject.objects.filter(class_ref=student.class_ref)
        subject_stats = []
        for subject in subjects:
            percent = calculate_attendance_percentage(student, subject, start_date, end_date)
            color = 'success' if percent >= 75 else ('warning' if percent >= 65 else 'danger')
            subject_stats.append({
                'subject': subject,
                'percent': percent,
        ...
        from academics.forms import StudentQueryForm
        from academics.models import Query
        query_form = StudentQueryForm(student=student)
        my_queries = Query.objects.filter(student=student).order_by('-created_at')
...
class StudentQuerySubmitView(StudentMixin, View):
    def post(self, request):
        from academics.forms import StudentQueryForm
        student = request.user
        form = StudentQueryForm(request.POST, student=student)
        if form.is_valid():
            query = form.save(commit=False)
            query.student = student
            query.institution = student.institution
            query.save()
            messages.success(request, 'Your question was successfully sent to the teacher!')
        else:
            messages.error(request, 'Failed to submit query. Please check if you selected a valid teacher.')
        return redirect('student_dashboard')

class TeacherQueryListView(TeacherMixin, View):
    def get(self, request):
        teacher = request.user
        from academics.models import Query
        queries = Query.objects.filter(teacher=teacher).order_by('status', '-created_at')
        return render(request, 'attendance/teacher_queries.html', {'queries': queries})
    ...
    def post(self, request):
        ...
        if query_id and answer:
            try:
                query = Query.objects.get(id=query_id, teacher=teacher)
                query.answer = answer
                query.status = 'answered'
                query.save()
                messages.success(request, f"Successfully answered {query.student.get_full_name()}'s query.")
            except Query.DoesNotExist:
                messages.error(request, "Query not found or unauthorized.")
        else:
            messages.error(request, "Invalid submission.")
            
        return redirect('teacher_queries')

class TeacherStudentCreateView(TeacherMixin, CreateView):
    model = User
    form_class = AddStudentForm
    template_name = 'attendance/teacher_student_form.html'
